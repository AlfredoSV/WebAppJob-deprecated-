...
BEGIN
    ...
    IF @textSearch <> ''
    BEGIN
            SELECT @count = COUNT(*) from Jobs where isactive = 1 and 
            namejob like  CONCAT('%', @textSearch,'%') or descriptionjob like  CONCAT('%', @textSearch,'%');

            SELECT * from Jobs where isactive = 1 and 
    ...
    ELSE
    BEGIN
            SELECT @count = COUNT(*) from Jobs where isactive = 1;
            SELECT * from Jobs where isactive = 1 order by namejob
            offset @pageSize*@page rows 
            fetch next @pageSize rows only;
    END;
